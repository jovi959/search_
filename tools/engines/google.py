"""
Google search using SeleniumBase UC mode.
...
def search(driver, query: str) -> list[dict]:
    ...
    """
    try:
        stealth_open(driver, "https://www.google.com/")
...
def _wait_for_results(driver, timeout: float = _RESULT_WAIT):
    ...
    while time.monotonic() < deadline:
        try:
            if (
                driver.find_elements("css selector", "div.g")
                or driver.find_elements("css selector", "a h3")
            ):
                return
        except Exception:
            pass
...
def _type_query(driver, query: str):
    ...
    for selector in ["textarea[name='q']", "input[name='q']"]:
        try:
            ...
            continue
    stealth_open(driver, f"https://www.google.com/search?q={quote_plus(query)}")
